...
HiHat density pot      ---> you need ARDCORE with 3 pots

X coordinate           ---> A2
Y coordinate           ---> A3 (don't ask me why i did it this way)
...
void setup() {
  ...
  pinMode(BDClockOut, OUTPUT);
  pinMode(SDClockOut, OUTPUT);  
  attachInterrupt(0, isr, RISING);
  pinMode(clkIn, INPUT);
  digitalWrite(BDClockOut, LOW);
  digitalWrite(SDClockOut, LOW);
...
void loop() {
  ...
  ///////////////////////////////// above are the equations for interpolating between values. Nothing fancy.
    ...
    if (clkState == HIGH) {
      if (BDrum > BDfreq) digitalWrite(BDClockOut, HIGH);
      if (SDrum > SDfreq) digitalWrite(SDClockOut, HIGH);
